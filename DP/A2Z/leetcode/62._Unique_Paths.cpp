...
public:
    int uniquePaths(int m, int n) {
        ...
        dp[0][0]=1;          // if there is only one block then reaching it have only 1 uniq path
        for(int i=0;i<n;i++){       // if only one row exist we can reach each block just by moving right so 1 uniq path
            dp[0][i]=1;
        }
        for(int j=0;j<m;j++){       // if only one column exist we can reach each blcok by moving down so 1 uniq path
            dp[j][0]=1;
        }
        for(int i=1;i<m;i++){        
            for(int j=1;j<n;j++){
        ...
        return dp[m-1][n-1];        // if m and n block we have we return this value 
    }
};
